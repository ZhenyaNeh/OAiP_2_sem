...
struct Node
{
    ...
    Node(int key, int count, char namePeople) : key(key), count(count), namePeople(namePeople), next(nullptr) {}
};
...
public:
    ...
    void add(char namePeople)
    {
        int key = (int)namePeople;
        int index = hash(key);
        int count{};
        Node* node = new Node(key, count, namePeople);
    ...
    void displayHashTable()
    {
        for (int i = 0; i < size; i++)
        {
            ...
            if (table[i] == nullptr)
            {
                ...
                {
                    cout << "[ " << currentNode->key << ", " << currentNode->namePeople << ", " << currentNode->count + 1<< "\t ] --> ";
